...
def train_agent(restore_prior_from='data/Prior.ckpt',
    ...
    experience = Experience(voc, max_size=1024)

    ga_handler = GeneticOperatorHandler(mutation_rate=0.01, 
    ...
    for step in range(n_steps):
        ...
        # TODO: re-normalize scores of collected samples (??)
        ...
        if population_size and len(experience.mol_buffer) > population_size:
            populations = tuple(map(list, zip(*[(smi, elem) for (smi, elem) in experience.mol_buffer.items()])))
            ...
            for g in range(ga_generations):
                child_smis, child_n_atoms, pop_smis, pop_scores = ga_handler.query(
                        query_size=offspring_size, mating_pool=populations, pool=pool, 
                        rank_coefficient=rank_coefficient, 
                    )

                # child_score = np.array(self.oracle(child_smis))
                child_fitness = scoring_function(child_smis)
                # normalize using sigmoid
                if step == 0:
                    child_score = normalize_score(child_fitness, collector['fitness'])     
                else:
                    child_score = normalize_score(child_fitness, results['fitness'].tolist())
                
                collector['smiles'] += smiles
                collector['fitness'] = np.concatenate([collector['fitness'], child_fitness]) 
                collector['score'] = np.concatenate([collector['score'], child_score]) 
        ...
        avg_loss = 0.
        if experience_replay and len(experience) > experience_replay:
            for _ in range(experience_loop):
                if rank_coefficient > 0:
                    exp_seqs, exp_score = experience.rank_based_sample(experience_replay, rank_coefficient)
                else:
                ...
                # print(loss.item())
                avg_loss += loss.item()/experience_loop
    ...
    torch.save(Agent.rnn.state_dict(), os.path.join(save_dir, 'Agent_genetic_gfn.ckpt'))
