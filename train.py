...
sys.path.append(os.path.join(path_here, 'genetic_gfn'))
...
from genetic_gfn.scoring_function import get_scores#, int_div#, get_original_docking_scores
...
class Genetic_GFN_trainer():
    ...
    def train(self):
        ...
        if self.oracle.startswith('tadf'):
            num_metric = 4
        else:
            num_metric = 2
        ...
        for step in tqdm(range(self.n_steps)):
            ...
            if self.population_size and len(self.memory) > self.population_size:
                ...
                population = (self.memory.smiles.tolist(), self.memory.scores.tolist())
                for g in range(self.ga_generations):
                    ...
                    self._memory_update(child_smis, child_score, seqs, child_all_scores[:, 1:])
            ...
            if len(self.memory) > 100:
                ...
                if self.wandb == 'online':
                    ...
                    print(f'top-100 score: {np.mean(np.array(self.memory["scores"][:100]))}')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--oracle', type=str, default="docking_1syh_qvina")
    ...
    parser.add_argument('--experience_replay', type=int, default=64)
    ...
    parser.add_argument('--prior_path', type=str, default="genetic_gfn/data/Prior_docking.ckpt")
    parser.add_argument('--vocab_path', type=str, default="genetic_gfn/data/Voc_docking")
    ...
    # set_seed(42)
    if args.oracle.startswith('tadf'):
        args.prior_path = "genetic_gfn/data/Prior_tadf.ckpt"
        args.voc_path = "genetic_gfn/data/Voc_tadf"
    elif args.oracle.startswith('docking'):
        args.prior_path = "genetic_gfn/data/Prior_docking.ckpt"
        args.voc_path = "genetic_gfn/data/Voc_docking"
    ...
    if args.wandb == 'online':
        wandb.init(project='tartarus', group=args.oracle.split('_')[0], config=args, reinit=True)
        if args.run_name == "":
